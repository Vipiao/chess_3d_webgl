...
void main(void) {
	...
	//vec3 normal = normalize(worldNormal + (surfaceColor) * 2.);
	...
	vec3 result;
	result = directionalLight(lightDir, viewDir, normal, diffuseFactor * brightness, specularFactor * brightness, ambientFactor * brightness,
	...
	// fog
	//float f = 1. - 1. / (1. + pow(length(worldPos - cameraPos) * 0.07, 1.5));
	float f = 1. - pow(0.97, length(worldPos - cameraPos));
...
vec3 directionalLight(vec3 lightDir, vec3 viewDir, vec3 normal, float diffuseFactor, float specularFactor, float ambientFactor,
	...
	spec *= (surfaceColor.r + surfaceColor.g + surfaceColor.b) / 3.;
	// fresnel
	float fresnelFactor = max(dot(normal, -viewDir), 0.0);
	float invFresnelFactor = 1. - fresnelFactor;
	
	// combine
	/*if(modulo(tick, 120.) < 60.){
		specularFactor *= invFresnelFactor;
		diffuseFactor *= invFresnelFactor;
		ambientFactor *= invFresnelFactor;
	}*/
